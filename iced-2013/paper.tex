...
questions along with research questions 7 and 8. We first examined the prevention / detection side of indirect conflicts through the use of
...
conflicts. As was found through RQ7, developers major concern with preemptive indirect conflict tools was the number of false positives put
...
changed pre-conditions in methods most times if not always, there is still a large chance that that changed pre-condition will actually not
...
precision of results. These types of tools are in direct comparison with the results of RQ8 in that developers were more interested in
...
\begin{description}
	\item[F7] It is extremely difficult, if not impossible, to create a preemptive indirect conflict tool which can detect indirect conflicts
						with a low rate of false positives.
	\item[F8] Indirect conflict tools are best used by providing exploratory environments to aid in the solving of indirect conflicts.
	\item[F9] Indirect conflict tools should be reducing debugging time of indirect conflicts as opposed to preventative
